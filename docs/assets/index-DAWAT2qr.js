(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`cybersec_lab_`,t={get(t,n=null){try{let r=localStorage.getItem(e+t);return r?JSON.parse(r):n}catch(e){return console.error(`Storage get error:`,e),n}},set(t,n){try{return localStorage.setItem(e+t,JSON.stringify(n)),!0}catch(e){return console.error(`Storage set error:`,e),!1}},remove(t){try{return localStorage.removeItem(e+t),!0}catch(e){return console.error(`Storage remove error:`,e),!1}},clear(){try{return Object.keys(localStorage).filter(t=>t.startsWith(e)).forEach(e=>localStorage.removeItem(e)),!0}catch(e){return console.error(`Storage clear error:`,e),!1}},keys(){return Object.keys(localStorage).filter(t=>t.startsWith(e)).map(t=>t.replace(e,``))}},n=`xp`,r=[0,100,250,500,800,1200,1800,2500,3500,5e3,7e3,1e4,14e3,19e3,25e3,32e3,4e4,5e4,65e3,8e4],i=[`Newbie`,`Script Kiddie`,`Junior Hacker`,`Security Analyst`,`Penetration Tester`,`Security Engineer`,`Bug Hunter`,`Red Teamer`,`Security Architect`,`Security Expert`,`Bug Bounty Hunter`,`CISO`,`Security Researcher`,`Cyber Warrior`,`Elite Hacker`,`Security Legend`,`Cyber God`,`1337 Hacker`,`Security Master`,`Ultimate Hacker`],a={getXP(){return t.get(n,0)},addXP(e){let r=this.getXP()+e;t.set(n,r);let i=this.getLevel(),a=this.getLevelFromXP(r);return a>i?{levelUp:!0,newLevel:a,xp:r}:{levelUp:!1,xp:r}},getLevelFromXP(e){let t=0;for(let n=0;n<r.length&&e>=r[n];n++)t=n+1;return t},getLevel(){let e=this.getXP();return this.getLevelFromXP(e)},getLevelTitle(){let e=this.getLevel();return i[Math.min(e-1,i.length-1)]},getProgress(){let e=this.getXP(),t=this.getLevel();if(t>=r.length)return{percentage:100,current:e,needed:0};let n=r[t-1]||0,i=r[t],a=e-n,o=i-n;return{percentage:Math.min(100,a/o*100),current:a,needed:o}},getBadgeText(){let e=this.getXP();return e>=1e3?`${(e/1e3).toFixed(1)}k XP`:`${e} XP`}},o=`アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワヲン0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ`,s=14,c=50;function l(){let e=document.getElementById(`matrix-bg`);if(!e)return;let t=e.getContext(`2d`);function n(){e.width=window.innerWidth,e.height=window.innerHeight}n(),window.addEventListener(`resize`,n);let r=Math.floor(e.width/c),i=Array(r).fill(1);function a(){t.fillStyle=`rgba(10, 10, 15, 0.05)`,t.fillRect(0,0,e.width,e.height),t.fillStyle=`#00ff41`,t.font=`${s}px monospace`;for(let n=0;n<i.length;n++){let r=o[Math.floor(Math.random()*82)];t.fillText(r,n*c,i[n]*s),i[n]*s>e.height&&Math.random()>.975&&(i[n]=0),i[n]++}}setInterval(a,50)}var u=3e3,d=`toast-container`,f={success:`✓`,error:`✗`,info:`ℹ`,warning:`⚠`,xp:`⚡`},p={success:`var(--success)`,error:`var(--danger)`,info:`var(--info)`,warning:`var(--warning)`,xp:`var(--warning)`};function m(e,t=`info`,n=u){let r=document.getElementById(d);if(!r)return;let i=document.createElement(`div`);i.className=`toast ${t}`,i.innerHTML=`
  ...
  `,document.body.appendChild(e);let n=document.getElementById(`access-code`),r=document.getElementById(`login-btn`),i=document.getElementById(`login-error`),a=document.getElementById(`guest-btn`);n.addEventListener(`keypress`,e=>{e.key===`Enter`&&o()}),r.addEventListener(`click`,o),a.addEventListener(`click`,()=>{w.isLoggedIn=!0,w.isGuest=!0,t.set(`isLoggedIn`,!0),t.set(`isGuest`,!0),e.remove(),E()});async function o(){let a=n.value.trim();if(!a){i.textContent=`Please enter an access code`,i.classList.add(`show`);return}r.disabled=!0,r.textContent=`VERIFYING...`,C.setToken(a);let o=await C.verifyToken();o.valid?(w.isLoggedIn=!0,w.isGuest=!1,t.set(`isLoggedIn`,!0),t.set(`isGuest`,!1),e.remove(),E(),m(`Welcome, ${o.user.login}!`,`success`)):a===`irfan123`||a===`admin`?(w.isLoggedIn=!0,w.isGuest=!1,t.set(`isLoggedIn`,!0),t.set(`isGuest`,!1),e.remove(),E(),m(`Welcome back, Irfan!`,`success`)):(C.removeToken(),i.textContent=`Invalid access code`,i.classList.add(`show`),r.disabled=!1,r.textContent=`ENTER`,n.value=``,n.focus())}}function E(){let e=document.getElementById(`navbar`);e&&e.classList.remove(`hidden`),l(),D(),Y(),O(`dashboard`)}function D(){let e=document.querySelectorAll(`.nav-link`),t=document.getElementById(`nav-toggle`),n=document.getElementById(`nav-links`);e.forEach(t=>{t.addEventListener(`click`,r=>{r.preventDefault();let i=t.dataset.page;e.forEach(e=>e.classList.remove(`active`)),t.classList.add(`active`),O(i),n.classList.remove(`active`)})}),t.addEventListener(`click`,()=>{n.classList.toggle(`active`)})}function O(e){w.currentPage=e;let t=document.getElementById(e);if(t)switch(e){case`dashboard`:k(t);break;case`path`:A(t);break;case`curriculum`:j(t);break;case`challenges`:F(t);break;case`cheatsheet`:z(t);break;case`projects`:H(t);break;case`admin`:U(t);break}}function k(e){let t=a.getXP(),n=a.getLevel(),r=a.getLevelTitle(),i=a.getProgress();e.innerHTML=`
    <div class="dashboard-welcome">
      <h1 class="welcome-text">Welcome back, <span class="text-white">Irfan</span></h1>
      <p class="welcome-subtitle">Level ${n} ${r} — ${t} XP accumulated</p>
    </div>
  ...
  `}function A(e){let t=w.curriculum.phases;e.innerHTML=`
    <h1 class="section-title">Learning Path</h1>
    <p class="mb-3 text-dim">Complete each phase to unlock the next.</p>
    
    <div class="path-container">
      ${t.map((e,n)=>{let r=q(e),i=e.status===`locked`&&n>0&&!J(t[n-1]);return`
          <div class="path-node ${i?`locked`:``} ${r===100?`completed`:``} ${e.status===`active`?`active`:``}" 
            ...
            <div class="path-node-header">
              ...
              <span class="path-node-status ${r===100?`status-completed`:e.status===`active`?`status-active`:`status-locked`}">
                ${r===100?`✓ DONE`:e.status===`active`?`► ACTIVE`:`LOCKED`}
              </span>
            </div>
            ...
            <div class="path-node-modules">
              ...
              ${e.modules.length>3?`<span class="module-tag">+${e.modules.length-3}</span>`:``}
            </div>
          </div>
          ${n<t.length-1?`<div class="path-connector"></div>`:``}
        `}).join(``)}
    </div>
  `}function j(e){let t=w.curriculum.phases;e.innerHTML=`
    <h1 class="section-title">Curriculum</h1>
    
    <div class="curriculum-tabs">
      ${t.map((e,t)=>`
        <button class="tab-btn ${t===0?`active`:``}" data-phase="${e.id}">
          ${e.icon} ${e.title}
        </button>
      `).join(``)}
    </div>
  ...
  `,e.querySelectorAll(`.tab-btn`).forEach(n=>{n.addEventListener(`click`,()=>{let r=n.dataset.phase,i=t.find(e=>e.id===r);e.querySelectorAll(`.tab-btn`).forEach(e=>e.classList.remove(`active`)),n.classList.add(`active`),document.getElementById(`module-list`).innerHTML=M(i),N()})}),N()}function M(e){return e.modules.map(e=>{let t=w.progress[e.id];return`
...
`)}</code></pre>
          ...
          <button class="btn btn-primary mt-2 complete-module-btn" data-module="${e.id}" data-xp="${e.xp}">
            ${t?`✓ Completed`:`Mark Complete`}
          </button>
        </div>
      </div>
    `}).join(``)}function N(){document.querySelectorAll(`.module-item`).forEach(e=>{e.addEventListener(`click`,t=>{if(t.target.closest(`.complete-module-btn`))return;let n=e.dataset.module;document.getElementById(`detail-${n}`).classList.toggle(`active`)})}),document.querySelectorAll(`.complete-module-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=e.dataset.module;P(n,parseInt(e.dataset.xp))})})}function P(e,n){if(w.progress[e])return;w.progress[e]=!0,t.set(`progress`,w.progress);let r=a.addXP(n);Y(),_(),m(`Module completed! +${n} XP`,`success`),r.levelUp&&setTimeout(()=>{m(`Level Up! Level ${r.newLevel}!`,`xp`)},1e3),O(w.currentPage)}function F(e){let t=w.challenges.challenges;e.innerHTML=`
  ...
  `,e.querySelectorAll(`.filter-btn`).forEach(n=>{n.addEventListener(`click`,()=>{let r=n.dataset.filter;e.querySelectorAll(`.filter-btn`).forEach(e=>e.classList.remove(`active`)),n.classList.add(`active`);let i=r===`all`?t:t.filter(e=>e.difficulty===r);document.getElementById(`challenges-grid`).innerHTML=i.map(e=>I(e)).join(``),L()})}),L()}function I(e){let t=w.solvedChallenges.includes(e.id);return`
    <div class="challenge-card ${t?`solved`:``}">
      <span class="challenge-difficulty">${e.difficulty}</span>
      <h3 class="challenge-title">${e.title}</h3>
      <p class="challenge-desc">${e.description}</p>
      <p class="challenge-xp">${e.xp} XP</p>
      
      ${t?`<p class="text-white mt-1" style="font-size: 0.8rem;">✓ Solved</p>`:`
        <div class="challenge-input">
          <input type="text" data-challenge="${e.id}" placeholder="Enter answer...">
          <button data-submit="${e.id}">Submit</button>
        </div>
        <div class="challenge-feedback" id="feedback-${e.id}"></div>
      `}
    </div>
  `}function L(){document.querySelectorAll(`[data-submit]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.submit;R(t)})}),document.querySelectorAll(`[data-challenge]`).forEach(e=>{e.addEventListener(`keypress`,t=>{t.key===`Enter`&&R(e.dataset.challenge)})})}function R(e){let n=w.challenges.challenges.find(t=>t.id===e),r=document.querySelector(`[data-challenge="${e}"]`),i=document.getElementById(`feedback-${e}`),o=r.value.trim().toLowerCase(),s=n.answer.toLowerCase(),c=(n.altAnswers||[]).map(e=>e.toLowerCase());if(o===s||c.includes(o)){w.solvedChallenges.push(e),t.set(`solvedChallenges`,w.solvedChallenges);let r=a.addXP(n.xp);Y(),i.className=`challenge-feedback show correct`,i.textContent=`✓ Correct! +${n.xp} XP`,_(),m(`Challenge solved! +${n.xp} XP`,`success`),r.levelUp&&setTimeout(()=>{m(`Level Up! Level ${r.newLevel}!`,`xp`)},1e3),setTimeout(()=>O(`challenges`),1500)}else i.className=`challenge-feedback show incorrect`,i.textContent=`✗ Incorrect. Try again.`}function z(e){let t=w.cheatsheet.categories,n=w.cheatsheet.cheats;e.innerHTML=`
    ...
    <div class="cheatsheet-categories">
      ${t.map(e=>`
        <button class="category-btn ${e===`All`?`active`:``}" data-category="${e}">
          ${e}
        </button>
      `).join(``)}
    </div>
  ...
  `,document.getElementById(`cheatsheet-search`).addEventListener(`input`,e=>{let t=e.target.value.toLowerCase(),r=n.filter(e=>e.command.toLowerCase().includes(t)||e.description.toLowerCase().includes(t));document.getElementById(`cheatsheet-list`).innerHTML=r.map(e=>B(e)).join(``),V()}),e.querySelectorAll(`.category-btn`).forEach(t=>{t.addEventListener(`click`,()=>{let r=t.dataset.category;e.querySelectorAll(`.category-btn`).forEach(e=>e.classList.remove(`active`)),t.classList.add(`active`);let i=r===`All`?n:n.filter(e=>e.category===r);document.getElementById(`cheatsheet-list`).innerHTML=i.map(e=>B(e)).join(``),V()})}),V()}function B(e){return`
    <div class="cheat-item">
      <div class="cheat-header">
        <code class="cheat-command">${e.command}</code>
        <span class="cheat-category">${e.category}</span>
      </div>
      <p class="cheat-desc">${e.description}</p>
    </div>
  `}function V(){document.querySelectorAll(`.cheat-command`).forEach(e=>{e.addEventListener(`click`,()=>{navigator.clipboard.writeText(e.textContent),e.classList.add(`copied`);let t=e.textContent;e.textContent=`Copied!`,setTimeout(()=>{e.classList.remove(`copied`),e.textContent=t},1e3)})})}function H(e){e.innerHTML=`
  ...
  `}function U(e){e.innerHTML=`
    ...
    `}
  `;let t=document.getElementById(`connect-btn`);t&&t.addEventListener(`click`,W);let n=document.getElementById(`disconnect-btn`);n&&n.addEventListener(`click`,G);let r=document.getElementById(`sync-btn`);r&&r.addEventListener(`click`,K),e.querySelectorAll(`.admin-tab`).forEach(t=>{t.addEventListener(`click`,()=>{e.querySelectorAll(`.admin-tab`).forEach(e=>e.classList.remove(`active`)),t.classList.add(`active`)})})}async function W(){let e=document.getElementById(`github-token`).value.trim(),t=document.getElementById(`connect-btn`);if(!e){m(`Please enter a token`,`error`);return}t.disabled=!0,t.textContent=`VERIFYING...`,C.setToken(e);let n=await C.verifyToken();n.valid?(m(`Connected as ${n.user.login}!`,`success`),O(`admin`)):(C.removeToken(),m(`Invalid token`,`error`),t.disabled=!1,t.textContent=`CONNECT`)}function G(){C.removeToken(),m(`Disconnected from GitHub`,`info`),O(`admin`)}async function K(){try{let e={user:`irfan`,lastUpdated:new Date().toISOString(),xp:a.getXP(),level:a.getLevel(),completedModules:w.progress,solvedChallenges:w.solvedChallenges};await C.commitProgress(e),m(`Progress synced to GitHub!`,`success`)}catch(e){m(`Failed to sync: `+e.message,`error`)}}function q(e){return e.modules.filter(e=>w.progress[e.id]).length/e.modules.length*100}function J(e){return e.modules.every(e=>w.progress[e.id])}function Y(){let e=document.getElementById(`xp-badge`);e&&(e.textContent=a.getBadgeText())}window.showPhaseDetail=e=>{console.log(`Phase clicked:`,e)},window.logout=()=>{t.remove(`isLoggedIn`),t.remove(`isGuest`),w.isLoggedIn=!1,location.reload()};